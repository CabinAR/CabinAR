...
function Viewport(inspector) {
  ...
  Events.on('entityupdate', details => { 
    var component = details.component

    if(
      (["position","rotation","scale"].indexOf(component) != -1) &&
      details.entity &&
      !details.entity.components[component]) {
      details.entity.setAttribute(
        component, 
        details.value
        )
    }
